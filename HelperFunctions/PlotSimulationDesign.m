...
%   Detailed explanation goes here
    ...
    subplot(323);
    image(scale01(rankTransform_equalsStayEqual(squareform(pdist(GroundTruth)),1)),'CDataMapping','scaled','AlphaData',~isnan(squareform(pdist(GroundTruth))));
    axis square off
    title('simulated ground truth RDM','FontWeight','bold')

    subplot(324);
    image(scale01(rankTransform_equalsStayEqual(Model.RDM,1)),'CDataMapping','scaled','AlphaData',~isnan(Model.RDM));
    axis square off
    ...
    handleCurrentFigure(fullfile(userOptions.rootPath,'SLsimulationSettings'),userOptions);
